...
export default function AdminLayout({ children, onLogout }) {
  ...
  return (
    <div className="min-h-screen flex bg-gradient-to-tr from-blue-600 to-blue-500 text-white font-sans">
      {/* Sidebar desktop */}
      <aside className="hidden md:flex w-64 bg-blue-700/95 backdrop-blur flex-col p-6 rounded-tr-3xl rounded-br-3xl shadow-2xl">
        {/* Brand */}
...
/** Kartu semangat dengan love animation */
function LoveNote({ className = "" }) {
  return (
    <div
      className={`relative bg-blue-800 rounded-xl p-4 text-sm shadow-lg overflow-hidden ${className}`}
    >
      <div className="flex items-start gap-3">
        <div className="relative">
          <AiFillHeart className="text-pink-400 heart-pulse" size={22} />
        </div>
        <div className="leading-snug">
          <p className="font-semibold">
            Halo Admin, <span className="text-yellow-300">semangat</span>{" "}
            kerjanya!
          </p>
        </div>
      </div>
      ...
      <div className="pointer-events-none absolute inset-0">
        <span className="floating-heart left-4 bottom-2">💗</span>
      </div>
      ...
      <style>{`
        ...
        .heart-pulse { animation: pulseHeart 1.6s ease-in-out infinite; }

        @keyframes floatUp {
          0%   { transform: translateY(0) scale(0.9); opacity: .85; }
          60%  { opacity: .75; }
        ...
        .floating-heart.delay-200 { animation-delay: .2s; }
        .floating-heart.delay-500 { animation-delay: .5s; }
        .floating-heart.delay-700 { animation-delay: .7s; }
      `}</style>
    </div>
  );
}
